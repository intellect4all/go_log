...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	snapshotStore, err := raft.NewFileSnapshotStore(filepath.Join(dataDir, "raft", "snap"), retain, os.Stderr)
	...
	if l.config.Raft.Config.HeartbeatTimeout != 0 {
...
func (l *DistributedLog) Read(off uint64) (*api.Record, error) {
	return l.log.Read(off)
}
...
func (s StreamLayer) Dial(address raft.ServerAddress, timeout time.Duration) (net.Conn, error) {
	...
	// identify to mux this is a raft connection

...
func testUnauthorized(t *testing.T, _, client api.LogClient, config *Config) {
	...
	if produce != nil {
		t.Fatalf("produce not nil: %v", produce)
	}
	...
	if consume != nil {
		t.Fatalf("consume not nil: %v", consume)
	}
...
func setupTest(t *testing.T, fn func(*Config)) (rootClient api.LogClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	...
	serverTLSConfig, err := config.SetupTLSConfig(config.TLSConfig{CAFile: config.CAFile, CertFile: config.ServerCertFile, KeyFile: config.ServerKeyFile, ServerAddress: l.Addr().String(), Server: true})
	require.NoError(t, err)
